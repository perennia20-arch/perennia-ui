<script lang="ts">
    ...
    import { dispatchStateAction, workers, sectors, systemMode, adminModeActive, adminTargetWallet, coreTokenRegistry, ClassThemes, globalKasPrice, globalKasChange, globalNetworkHashrate, globalNodeStatus, type Worker, type Sector, type RouteMode, type SettlementConfig, type TokenAsset, type AssetClass } from '$lib/stores/app';
    ...
    // ⚡ Custom UI Type to satisfy strict mode without crashing the Svelte template compiler
    ...
    let allocatedPercentage = $derived($sectors.reduce((acc, s) => acc + s.allocationPercentage, 0));
    let unallocatedPercentage = $derived(100.0 - allocatedPercentage);
    ...
    // ==========================================
    // THE SECTOR CANVAS & SETTLEMENT STATE
    // ==========================================
    ...
    function updateDraftHash(valStr: string) {
        ...
        draftSectorPct = parseFloat(((val / displayHashrate) * 100).toFixed(1));
    }
    ...
    function handleSectorInputHash(id: string, hashStr: string) {
        ...
        sectors.update(arr => arr.map(s => {
            if (s.id === id) {
                ...
                const pct = parseFloat(((cleanHash / displayHashrate) * 100).toFixed(1));
        ...
        try {
            ...
            if (lsData) {
                ...
                dynamicTreasuryTokens = parsed.map((t: any) => ({
                    ...
                    priceUsd: t.price || 0,
                    imgUrl: t.imgUrl || `https://storage.googleapis.com/kasfyi/token-icons/${t.symbol}.png`,
                    icon: t.icon,
    ...
    async function saveSettlementParams() { 
        if (settlementModalSilo) { 
            ...
            try {
                ...
                // Deep equality check: If ONLY the name changed, skip the signature prompt.
    ...
    async function fetchSyntheticLedger() {
        ...
        } catch (e) { console.error("Operations Ledger Hydration Failed:", e); }
    }
    ...
    $effect(() => {
        if (browser && $isWalletConnected && $walletAddress) {
            ...
            const unsubscribe = connection.select('message').subscribe((rawData) => {
                ...
                } catch(e) { globalNodeStatus.set('unreachable'); }
            });
    ...
    <div draggable="true"
        ...
        <div class="flex justify-between items-start">
            <div class="flex flex-col">
                ...
                <!-- ⚡ RESTORED NATIVE INLINE RENAMING (NO SIGNATURE) -->
            ...
            <div class="flex items-center gap-3">
                {#if isLp}
                    <div class="flex -space-x-2">
                        ...
                        <div class="w-6 h-6 rounded-full border border-neutral-800 bg-[#050505] flex items-center justify-center text-[8px] font-black text-blue-400">{sector.settlementConfig.targetAsset.ticker[0]}</div>
                    </div>
                    <span class="text-xs font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#18C6A5] to-[#3B82F6] tracking-widest">KAS / {sector.settlementConfig.targetAsset.ticker}</span>
                {:else}
                    <div class="w-6 h-6 rounded-full border border-neutral-800 bg-[#050505] flex items-center justify-center text-[10px] font-black overflow-hidden" style={`color: ${hex};`}>
                        ...
                        {:else}
                            {sector.settlementConfig.targetAsset.ticker[0]}
                        {/if}
                    </div>
                    <span class="text-xs font-bold tracking-widest" style={`color: ${hex};`}>{sector.settlementConfig.targetAsset.ticker}</span>
                {/if}
            </div>
            <div class="flex flex-col items-end">
        ...
        <div class="mt-2 flex flex-col gap-2">
            ...
            <input type="range" 
                   ...
                   class="w-full custom-range-slider" />
        </div>
    </div>
{/snippet}

<div class="w-full h-full p-4 md:p-6 lg:p-10 overflow-y-auto bg-[#030303]" role="presentation">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 xl:gap-6 w-full max-w-[1600px] mx-auto pb-24 items-start">
        ...
        <div class="flex flex-col gap-4 lg:col-span-3 xl:col-span-2">
            ...
            <div class="flex flex-col gap-2 mt-2">
                <div class="flex items-center justify-between border-b border-neutral-800/80 pb-2 mb-2">
                    <h2 class="text-[10px] font-bold uppercase tracking-widest text-neutral-500">Fleet Diagnostics</h2>
                    <span class="text-[9px] font-mono text-neutral-600 font-bold">{$workers.length} RIGS</span>
                </div>
                ...
                {#each displayedWorkers as worker (worker.id)}
                    <div class="bg-[#0a0a0a] border border-neutral-800/80 rounded-xl p-3 flex flex-col gap-2 relative overflow-hidden shadow-sm">
                        <div class="flex justify-between items-center">
        ...
        <!-- ======================================================= -->
        <!-- MIDDLE COLUMN: CHARTS & SECTOR CANVAS                   -->
        <!-- ======================================================= -->
...
{#if isSectorModalOpen}
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#050505]/95" transition:fade={{ duration: 150 }}>
        <!-- Disabled background click to close -->
        ...
        <div class="relative z-10 w-full max-w-[480px] bg-[#0c0c0c] border border-neutral-700 rounded-[28px] overflow-hidden shadow-[0_0_50px_rgba(255,255,255,0.05)] flex flex-col animate-[fade-in-up_0.2s_ease-out]">
            ...
            <div class="p-6 border-b border-neutral-800 bg-[#111] flex justify-between items-center">
                <div class="flex flex-col">
            ...
            <div class="p-6 flex flex-col gap-6 max-h-[75vh] overflow-y-auto hide-scrollbar">
                
                <!-- Sector Identifier Name -->
                ...
                <div class="flex flex-col gap-4">
                    ...
                    {:else if draftSettlementConfig.mode === 'appointment'}
                        <div class="flex flex-col gap-3 animate-[fade-in-up_0.2s_ease-out] {draftSettlementConfig.autoPayout ? '' : 'opacity-50 pointer-events-none'}">
                            <span class="text-[9px] text-neutral-500 uppercase tracking-widest font-bold ml-1 block">Chronos Scheduling</span>
...
{#if settlementModalSilo && editSettlementParams}
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative z-10 w-full max-w-[420px] bg-[#111] border border-neutral-800 rounded-[28px] shadow-2xl flex flex-col overflow-hidden transition-colors duration-500 border-t-4 {editSettlementParams.autoPayout ? (editSettlementParams.mode === 'stream' ? 'border-t-emerald-500' : editSettlementParams.mode === 'appointment' ? 'border-t-blue-500' : 'border-t-amber-500') : 'border-t-neutral-600'} animate-[fade-in-up_0.2s_ease-out]">
            ...
            <div class="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto hide-scrollbar">

                <!-- Sector Identifier / Rename -->
                <div class="flex flex-col gap-1.5">
...
<!-- ========================================== -->
<!-- ⚡ UNIFIED ASSET SELECTOR OVERLAY MODAL     -->
<!-- ========================================== -->
